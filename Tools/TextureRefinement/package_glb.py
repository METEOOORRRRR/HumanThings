...
doc['images'][1]['name']='ToxicBunny_Clean_BaseColor'
normal_path=pathlib.Path(sys.argv[3]) if len(sys.argv)>3 else None
if normal_path:
    while len(binary)%4:binary+=b'\0'
...
baseline=json.loads((QA/'source_inspection.json').read_text())['sha256']
