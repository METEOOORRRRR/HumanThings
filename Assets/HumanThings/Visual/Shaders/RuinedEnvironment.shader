Shader "HumanThings/Ruined Environment"
{
    Properties
    {
        _Cracks("Crack Strength", Range(0,1)) = 0.65
        _Scorch("Scorch Strength", Range(0,1)) = 0.7
        _DamageScale("Damage Scale", Float) = 1
    ...
    SubShader
    {
        ...
        float _Cracks, _Scorch, _DamageScale;
        ...
        half4 Frag(Varyings i):SV_Target
        {
            ...
            color*=1-wet*.16;
            // Broad fire scars and sparse fractures retain the original surface and geometry.
            float3 p=i.positionWS;
            half4 macro=Detail(p*.075+float3(7,3,11),n);
            float2 plane=abs(n.y)>.6 ? p.xz : abs(n.x)>abs(n.z) ? p.zy : p.xy;
            half patch=smoothstep(.38,.66,macro.r);
            half crack=Fracture(plane*_DamageScale+(d.rg-.5)*.16)*patch*_Cracks;
            half scorch=smoothstep(.40,.65,macro.g+(.5-d.b)*.2)*_Scorch;
            color=lerp(color,color*half3(.18,.16,.14),scorch);
            color=lerp(color,half3(.018,.019,.019),crack*.85);
            color=lerp(color,color*half3(.72,.54,.37),smoothstep(.25,.5,scorch)*(1-smoothstep(.5,.7,scorch))*.25);
            ...
            surface.smoothness=lerp(lerp(_Smoothness,.48,wet),.035,scorch); surface.occlusion=1-grime*.18;
